  ...
  <div>
      ...
      <Col span="24">
        <Card>
          <p slot="title" style="height: 32px">
            ...
            <Button type="warning" class="margin-left-5" @click="confirmRefresh = true" icon="md-refresh">一键能量盒收益</Button>
          </p>
          <div>
    ...
    <Modal v-model="modalSetting.show" width="668" :styles="{top: '30px'}" @on-visible-change="doCancel">
      ...
      <Form ref="myForm" :rules="ruleValidate" :model="formItem" :label-width="120">
        ...
        <FormItem label="需要爱心果" prop="price">
          <Input style="width: 300px" type="number" v-model="formItem.price"
                 placeholder="请输入需要爱心果数量">
          </Input>
          <Tag class="margin-left-5">（个）</Tag>
        </FormItem>
        <FormItem label="周期" prop="cycle">
          <Input style="width: 300px" type="number" v-model="formItem.cycle"
                 placeholder="请输入周期">
          </Input>
          <Tag class="margin-left-5">（天）</Tag>
        </FormItem>
        ...
        <FormItem label="奖励爱心值" prop="award_score">
          <Input style="width: 300px" type="number" v-model="formItem.award_score"
                 placeholder="请输入奖励爱心值">
          </Input>
        </FormItem>
        <FormItem label="推荐奖励爱心值" prop="award_share">
          <Input style="width: 300px" type="number" v-model="formItem.award_share"
                 placeholder="请输入推荐人奖励爱心值">
          </Input>
          <Tag class="margin-left-5">%</Tag>
        </FormItem>
    ...
    <Modal v-model="confirmRefresh" width="360">
      <p slot="header" style="color:#f60;text-align:center">
        ...
        <span>确定要一键收益能量盒</span>
      </p>
      <div style="text-align:center">
        <p>一键收益能量盒是一个非常危险的操作，它有可能影响到您所有会员的能量值收益，请确认无误后操作！！</p>
      </div>
      <div slot="footer">
        <Button type="error" size="large" long  :loading="refreshLoading" @click="productProfit">确定刷新</Button>
      </div>
    </Modal>
  </div>
</template>
<script>
import { getList, changeStatus, add, edit, del, doProductProfit } from '@/api/product'
    ...
    on: {
      'click': () => {
        getAll().then(response => {
          ...
          vm.formItem.award_score = currentRow.award_score
          vm.formItem.award_share = currentRow.award_share
  ...
  data () {
    return {
      ...
      ruleValidate: {
        ...
        price: [
          { required: true, message: '需要爱心果数量不能为空', trigger: 'blur' }
        ],
        energy_value: [
