          ...
          <Form inline>
            <FormItem style="margin-bottom: 0">
              <Select v-model="searchConf.status" clearable placeholder="请选择订单状态" style="width:100px">
                ...
                <Option :value="1">待确认收款</Option>
                <Option :value="2">投诉中</Option>
              </Select>
            </FormItem>
            <FormItem style="margin-bottom: 0">
...
<script>
import { getTradingList, sellCancel, cktp } from '@/api/exchange'
...
const deleteButton = (vm, h, currentRow, index) => {
  return h('Poptip', {
    ...
    on: {
      'on-ok': () => {
        sellCancel(currentRow.order_no).then(response => {
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      getTradingList(params).then(response => {
